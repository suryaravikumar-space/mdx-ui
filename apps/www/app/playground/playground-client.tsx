...
import {
  ...
  CardContent, CardFooter, LinkCard,
...
import { Solution, SolutionStep, SolutionAnswer, SolutionNote } from "@/components/mdx/math-solution"
import { ME, BME } from "@/components/mdx/math-easy"
import {
  DSArray, DSMatrix, DSLinkedList, DSDoublyLinkedList, DSCircularLinkedList,
  DSStack, DSQueue, DSDeque, DSPriorityQueue, DSBinaryTree, DSBST,
  DSMinHeap, DSMaxHeap, DSTrie, DSGraph, DSDigraph, DSWeightedGraph,
  DSDAG, DSHashMap, DSHashSet, DSNaryTree, DSSegmentTree, DSBFS, DSDFS, DSAgentFlow,
} from "@/components/mdx/ds"

const COMPONENTS: Record<string, React.ComponentType<any>> = {
  Accordion, AccordionItem, AccordionTrigger, AccordionContent,
  Badge, Blockquote, Callout, Steps, Step,
  Tabs, TabsList, TabsTrigger, TabsContent,
  Tree, TreeItem, FileTree, Headings,
  Paragraph, Lead, Intro, Large, Small, Muted,
  Math, BlockMath, InlineMath, M, BM,
  Mermaid, MermaidBST, MermaidTree, MermaidBFS, MermaidDFS,
  Table, TableHeader, TableBody, TableFooter, TableRow, TableHead, TableCell, TableCaption,
  CodeGroup,
  Alert, AlertTitle, AlertDescription,
  Card, CardHeader, CardTitle, CardDescription, CardContent, CardFooter, LinkCard,
  Kbd, Link, Spoiler, Annotation, Reveal,
  GlossaryProvider, Term,
  DataTable, Highlight, DiffBlock,
  Terminal, TerminalLine,
  Changelog, ChangelogEntry, ChangelogItem,
  Definition, Invariant, ComplexityTable, HardwareSpec,
  DataTypeTable, SecurityNote, RegisterMap, PrivacyTable, CertificationBadge,
  ...MathPrimitives,
  Equation, EqSystem,
  ...
  li: (props: React.HTMLAttributes<HTMLElement>) => <li className="mt-2" {...props} />,
...
export function PlaygroundClient({ initialCompiled }: PlaygroundClientProps = {}) {
  const [source, setSource] = useState(DEFAULT_MDX)
  const [compiled, setCompiled] = useState<MDXRemoteSerializeResult | null>(initialCompiled ?? null)
    ...
    <div className="flex h-[calc(100vh-3.5rem)] flex-col">
      ...
      <div className="flex flex-wrap items-center gap-3 border-b border-border bg-background/95 px-4 py-2 backdrop-blur">
        <div className="flex items-center gap-2.5">
          ...
          {!isCompiling && error && (
            <span className="font-mono text-xs text-destructive">syntax error</span>
          )}
        </div>
      ...
      <div className="flex min-h-0 flex-1">
        ...
        >
          <div className="border-b border-border bg-muted/40 px-3 py-1.5 flex items-center justify-between">
            <span className="font-mono text-xs text-muted-foreground">editor.mdx</span>
            <span className="font-mono text-xs text-muted-foreground/60">paste your LLM output here</span>
          </div>
          <textarea
            value={source}
          ...
          <div className="flex-1 overflow-y-auto">
            {error ? (
              ...
              <div className="mdx px-6 py-6">
                <MDXRemote {...compiled} components={COMPONENTS as any} />
              </div>
            ) : (
              <div className="flex h-full items-center justify-center">
                <span className="animate-pulse font-mono text-xs text-muted-foreground">compiling…</span>
              </div>
            )}
          </div>
        </div>
