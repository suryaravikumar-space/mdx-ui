...
export function Mermaid({
  ...
  if (!svg) {
    return (
      <div
        className={cn(
          "my-4 flex h-32 items-center justify-center rounded-lg border bg-muted/30",
          className,
        )}
      >
 ...
 * />
 */
export function MermaidBFS({
  nodes,
  edges,
  start,
  directed = false,
  className,
}: MermaidTraversalProps) {
  const order = computeBFS(nodes, edges, start, directed);
  const chart = graphToMermaid(nodes, edges, order, directed);
  return (
    <Mermaid chart={chart} label="BFS Traversal" beta className={className} />
  );
}
...
export function MermaidDFS({
  ...
  const chart = graphToMermaid(nodes, edges, order, directed);
  return (
    <Mermaid chart={chart} label="DFS Traversal" beta className={className} />
  );
}
