...
```bash
npx docsui-cli@latest add code-block
```
...
The CSS for Shiki token colors is automatically injected into your `globals.css` by `npx docsui-cli@latest init`:

```css
/* Shiki dual-theme — injected by docsui-cli init */
