...
<GlossaryProvider terms={{ BST: "Binary Search Tree — a tree where each node's left child is smaller and right child is larger." }}>
...
<GlossaryProvider
  terms={{
    ...
    QAIRT: "Qualcomm AI Runtime — the inference engine for Qualcomm AI hardware.",
  }}
>
...
export default function Layout({ children }) {
  return (
    <GlossaryProvider terms={pageTerms}>
      {children}
    </GlossaryProvider>
  )
}
