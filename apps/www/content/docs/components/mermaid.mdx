...
```bash
npx docsui-cli@latest add mermaid
```
...
When using `@docsui-cli/remark-plugin`, fenced ` ```mermaid ` code blocks are automatically converted to `<Mermaid>` components. This is **on by default**.
