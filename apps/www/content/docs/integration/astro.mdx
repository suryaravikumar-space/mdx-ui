...
```ts title="src/components/mdx-ui/mdx-components.ts"
...
<Callout variant="info" title="Tip">
  Components registered in mdxComponents are available here — no imports needed in the MDX file.
</Callout>
```
...
<Callout variant="info" title="Note">This is purely visual.</Callout>
...
<Callout variant="warning" title="Don't add client: to static components">
  Adding `client:load` to a purely visual component like `Callout` or `CodeBlock`
  ships unnecessary JavaScript. Only use it on components that have state or event
  handlers (Accordion, Tabs, Reveal).
</Callout>
