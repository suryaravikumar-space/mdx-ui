...
The DocsUI MCP server plugs directly into Claude Code (or any MCP-compatible AI client). Once connected, Claude can browse your component registry, look up math symbols, convert LaTeX expressions, and generate rich MDX content that renders correctly the first time — no post-editing.
...
The server starts automatically when you open a chat in Claude Code. You should see `docsui` listed under active MCP servers in the sidebar.
...
```
What components does DocsUI have?
```
