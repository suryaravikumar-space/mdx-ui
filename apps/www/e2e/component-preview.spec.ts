...
test.describe("Demo widget", () => {
  ...
  test.describe("accordion component page", () => {
    ...
    test("Preview tab button is visible and active by default", async ({
      page,
    }) => {
    ...
    test("clicking Code tab reveals syntax-highlighted source", async ({
      page,
    }) => {
      await page
      ...
      const codeContent = page
        .locator(".component-preview-code, pre code")
        .first();
    ...
    test("language label shown on Code tab", async ({ page }) => {
      await page
        ...
        .click();
      await expect(
        page.getByText("tsx", { exact: true }).first(),
      ).toBeVisible();
    });
  ...
  for (const component of COMPONENT_PAGES) {
    test(`${component}: page loads and has Preview+Code tabs`, async ({
      page,
    }) => {
      const response = await page.goto(`/docs/components/${component}`);
      expect(
        response?.status(),
        `${component} page returned ${response?.status()}`,
      ).not.toBe(404);
      await expect(
