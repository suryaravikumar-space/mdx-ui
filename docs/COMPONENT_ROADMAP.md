...
```mdx
<Lead>This guide will teach you the fundamentals of React.</Lead>
```
...
```mdx
<Summary>
  - You learned about hooks - State management is key - Effects run after render
</Summary>
```
...
```mdx
<Console>Warning: Each child should have a unique key prop</Console>
```
...
<Challenge title="Fix the bug">
  <Challenge.Problem>The counter doesn't increment. Fix it!</Challenge.Problem>
  <Challenge.Solution>Add setState to the onClick handler</Challenge.Solution>
</Challenge>
```
