...
describe("COMPONENT_FILES", () => {
  ...
  it("all file entries end with a valid extension", () => {
    ...
    for (const files of Object.values(COMPONENT_FILES)) {
      for (const f of files) {
        expect(validExts.some(ext => f.endsWith(ext)), `"${f}" has unexpected extension`).toBe(true)
      }
    }
  })
...
describe("COMPONENT_MDX_MAP", () => {
  ...
  it("every entry has required shape", () => {
    for (const [name, mapping] of Object.entries(COMPONENT_MDX_MAP)) {
      expect(mapping, `${name} mapping missing importFile`).toHaveProperty("importFile")
      expect(mapping, `${name} mapping missing imports`).toHaveProperty("imports")
      expect(mapping, `${name} mapping missing elementMappings`).toHaveProperty("elementMappings")
      expect(Array.isArray(mapping.imports), `${name}.imports should be array`).toBe(true)
  ...
  it("importFile starts with ./ (relative path)", () => {
    for (const [name, mapping] of Object.entries(COMPONENT_MDX_MAP)) {
      expect(mapping.importFile.startsWith("./"), `${name}.importFile should be relative`).toBe(true)
    }
  })

  it("imports array is non-empty", () => {
    for (const [name, mapping] of Object.entries(COMPONENT_MDX_MAP)) {
      expect(mapping.imports.length, `${name}.imports should not be empty`).toBeGreaterThan(0)
    }
  })
})
...
describe("FILE_TO_COMPONENT", () => {
  ...
  it("all keys end with .tsx or .ts", () => {
    for (const key of Object.keys(FILE_TO_COMPONENT)) {
      expect(key.endsWith(".tsx") || key.endsWith(".ts"), `"${key}" should be .tsx or .ts`).toBe(true)
    }
  })
  ...
  it("all values are non-empty strings", () => {
    for (const [file, component] of Object.entries(FILE_TO_COMPONENT)) {
      ...
      expect(component.length, `${file} maps to empty string`).toBeGreaterThan(0)
    }
  })
    ...
    for (const [componentName, files] of Object.entries(COMPONENT_FILES)) {
      for (const file of files) {
        if (file.endsWith(".tsx") || file.endsWith(".ts")) {
          expect(FILE_TO_COMPONENT[file], `${file} missing from FILE_TO_COMPONENT`).toBe(componentName)
        }
      }
    }
  })
