...
import { detectFramework, detectProjectStructure } from "../utils/detect-structure.js"
...
describe("detectProjectStructure", () => {
  ...
  it("sets hasTypeScript=true when tsconfig.json exists", async () => {
    mockPathExists.mockImplementation(async (p: unknown) => String(p).endsWith("tsconfig.json") as never)
    mockReadJSON.mockResolvedValue({ dependencies: {} } as never)
  ...
  it("sets hasTailwind=true when tailwind.config.ts exists", async () => {
    mockPathExists.mockImplementation(async (p: unknown) => (String(p).includes("tailwind.config.ts")) as never)
    mockReadJSON.mockResolvedValue({ dependencies: {} } as never)
