...
describe("CertificationBadge", () => {
  ...
  it("renders scope when provided", () => {
    render(<CertificationBadge certs={certs} />);
    expect(
      screen.getByText("Information Security Management"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Automotive Information Security"),
    ).toBeInTheDocument();
  });
  ...
  it("defaults missing status to active", () => {
    render(<CertificationBadge certs={[{ name: "GDPR" }]} />);
    expect(screen.getByText("Active")).toBeInTheDocument();
  });

  it("renders expired status", () => {
    render(
      <CertificationBadge certs={[{ name: "Old Cert", status: "expired" }]} />,
    );
  ...
  it("does not render year or dot separator when year is absent", () => {
    render(
      <CertificationBadge
        certs={[{ name: "SOC 2 Type II", status: "pending" }]}
      />,
    );
  ...
  it("renders a single cert without errors", () => {
    render(
      <CertificationBadge
        certs={[{ name: "ISO 9001", year: 2022, status: "active" }]}
      />,
    );
