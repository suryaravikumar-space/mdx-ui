...
const terms = {
  bfs: {
    label: "BFS",
    definition: "Breadth-First Search: explores level by level.",
  },
  dfs: {
    label: "DFS",
    definition: "Depth-First Search: explores as far as possible.",
  },
};
...
describe("GlossaryProvider + Term", () => {
  ...
  it("renders custom children text instead of label", () => {
    ...
    expect(
      screen.getByRole("button", { name: "breadth-first search" }),
    ).toBeInTheDocument();
  });
  ...
  it("degrades gracefully when term id is not in provider", () => {
    render(
      <GlossaryProvider terms={{}}>
        <Term id="unknown">fallback text</Term>
      </GlossaryProvider>,
    );
  ...
  it("multiple terms work independently", async () => {
    ...
    expect(
      screen.queryByText("Depth-First Search: explores as far as possible."),
