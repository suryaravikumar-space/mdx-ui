...
describe("PrivacyTable", () => {
  ...
  it("shows Yes for shared=true", () => {
    render(<PrivacyTable rows={rows} />);
    expect(
      screen.getByLabelText("shared with third parties"),
    ).toBeInTheDocument();
  });
  ...
  it("renders caption when provided", () => {
    render(
      <PrivacyTable rows={rows} caption="Per Qualcomm Privacy Policy v2.4" />,
    );
    expect(
      screen.getByText("Per Qualcomm Privacy Policy v2.4"),
    ).toBeInTheDocument();
  });
