...
export interface DataTableProps<T extends Record<string, unknown> = Record<string, unknown>> {
   ...
   */
  headers?: Array<string | { label: string; sortType?: "string" | "number" | "date" }>
...
function cmp(a: unknown, b: unknown, type: "string" | "number" | "date"): number {
...
function sortLabel(type: "string" | "number" | "date", dir: "asc" | "desc"): string {
...
function SortBadge({ dir, type }: { dir: SortDir; type: "string" | "number" | "date" }) {
  if (!dir) return (
...
function PaginationBar({
  total, page, pageSize, options, onPage, onSize,
}: {
  total: number; page: number; pageSize: number
  options: number[]; onPage: (p: number) => void; onSize: (n: number) => void
}) {
  ...
  const btn = "inline-flex h-8 min-w-[2rem] items-center justify-center rounded px-1.5 text-sm transition-colors"
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 pt-3 text-sm text-muted-foreground">
      ...
      <div className="flex items-center gap-1">
        ...
        <button onClick={() => onPage(page - 1)} disabled={page === 1}
          className={cn(btn, "border hover:bg-muted disabled:opacity-40")} aria-label="Previous">
          <Chevron d="left" />
        </button>
        {pages.map((p, i) =>
          p === "…"
            ? <span key={`e${i}`} className={cn(btn, "cursor-default")}>…</span>
            : <button key={p} onClick={() => onPage(p as number)}
                className={cn(btn, p === page ? "border border-ring bg-background font-semibold text-foreground" : "hover:bg-muted")}>
                {p}
              </button>
        )}
        <button onClick={() => onPage(page + 1)} disabled={page === totalPages}
          className={cn(btn, "border hover:bg-muted disabled:opacity-40")} aria-label="Next">
          <Chevron d="right" />
        </button>
      </div>
    </div>
...
export function DataTable<T extends Record<string, unknown>>({
  ...
  const { ncols, nrows } = React.useMemo<{ ncols: NCol[]; nrows: NRow[] }>(() => {
    // Simple API
    if (headers && rows) {
      ...
      nc.forEach((col, i) => {
        if (col.sortable && !(typeof headers[i] === "object" && (headers[i] as any).sortType)) {
  ...
  React.useEffect(() => { setPage(1) }, [search, sortKey, sortDir])
      ...
      <div className="w-full overflow-x-auto rounded-lg border">
        <table className="w-full caption-bottom text-sm">
          ...
          <tbody className="[&_tr:last-child]:border-0">
            {paged.length === 0 ? (
              <tr>
                <td colSpan={ncols.length} className="px-4 py-8 text-center text-muted-foreground">
                  {emptyMsg}
                </td>
              </tr>
            ) : (
                ...
                <tr key={ri} className="border-b transition-colors hover:bg-muted/50">
                  {ncols.map((col) => (
                    <td key={col.key} className={cn("px-4 py-3 align-middle", col.className)}>
                      {col.render
                        ? col.render(row[col.key], row, (page - 1) * pageSize + ri)
                        : String(row[col.key] ?? "")}
                    </td>
                  ))}
                </tr>
