...
export type TraversalType = "bfs" | "inorder" | "preorder" | "postorder" | "dfs"
...
export function validateBinaryTree(root: TreeNodeData): ValidationResult {
  ...
  check(root, "r")
  if (invalidNodeIds.size) violatedRules.push("Binary tree: each node may have at most 2 children")
  return { valid: !violatedRules.length, violatedRules, invalidNodeIds, invalidEdgeIds: new Set() }
}
...
export function validateBST(root: TreeNodeData): ValidationResult {
  ...
  if (invalidNodeIds.size) violatedRules.push("BST: left subtree values < node < right subtree values (recursively)")
  return { valid: !violatedRules.length, violatedRules, invalidNodeIds, invalidEdgeIds: new Set() }
}
...
export function validateAVL(root: TreeNodeData): ValidationResult {
  ...
  function check(node: TreeNodeData, id: string) {
    ...
    if (Math.abs(bf) > 1) {
      invalidNodeIds.add(id)
      violatedRules.push(`AVL: node "${node.value}" has balance factor ${bf} (|bf| must be ≤ 1)`)
    }
    if (node.left)  check(node.left,  `${id}L`)
    if (node.right) check(node.right, `${id}R`)
  }
...
export function validateBalanced(root: TreeNodeData): ValidationResult {
  ...
  check(root, "r")
  if (invalidNodeIds.size) violatedRules.push("Balanced: height difference between subtrees must be ≤ 1")
  return { valid: !violatedRules.length, violatedRules, invalidNodeIds, invalidEdgeIds: new Set() }
}
...
export function validateFull(root: TreeNodeData): ValidationResult {
  ...
  const violatedRules: string[] = []
  function check(node: TreeNodeData, id: string) {
    const hasLeft = !!node.left, hasRight = !!node.right
    if (hasLeft !== hasRight) invalidNodeIds.add(id)
    if (node.left)  check(node.left,  `${id}L`)
    if (node.right) check(node.right, `${id}R`)
  }
  check(root, "r")
  if (invalidNodeIds.size) violatedRules.push("Full binary tree: every node must have 0 or 2 children")
  return { valid: !violatedRules.length, violatedRules, invalidNodeIds, invalidEdgeIds: new Set() }
}
...
export function validateComplete(root: TreeNodeData): ValidationResult {
  ...
  while (queue.length) {
    ...
    if (node.left) {
      if (end) { invalidNodeIds.add(`${id}L`); violatedRules.push("Complete: gap found before last level fills left-to-right") }
      queue.push({ node: node.left,  id: `${id}L` })
    } else { end = true }
    if (node.right) {
      if (end) { invalidNodeIds.add(`${id}R`); violatedRules.push("Complete: gap found before last level fills left-to-right") }
      queue.push({ node: node.right, id: `${id}R` })
    } else { end = true }
  }
  return { valid: !violatedRules.length, violatedRules: [...new Set(violatedRules)], invalidNodeIds, invalidEdgeIds: new Set() }
}
...
export function validatePerfect(root: TreeNodeData): ValidationResult {
  ...
  function check(node: TreeNodeData, id: string, depth: number) {
    ...
    if (isLeaf && depth !== targetDepth) { invalidNodeIds.add(id); }
    if (!isLeaf && (!node.left || !node.right)) { invalidNodeIds.add(id) }
    if (node.left)  check(node.left,  `${id}L`, depth + 1)
    if (node.right) check(node.right, `${id}R`, depth + 1)
  }
  check(root, "r", 0)
  if (invalidNodeIds.size) violatedRules.push("Perfect: all leaves must be at same depth; all internal nodes must have 2 children")
  return { valid: !violatedRules.length, violatedRules, invalidNodeIds, invalidEdgeIds: new Set() }
}

export function validateHeap(root: TreeNodeData, type: "min" | "max"): ValidationResult {
  const invalidNodeIds = new Set<string>()
  const violatedRules: string[] = []
  function check(node: TreeNodeData, id: string) {
    const v = Number(node.value)
    if (node.left)  { const lv = Number(node.left.value);  if (type === "min" ? v > lv : v < lv) invalidNodeIds.add(`${id}L`); check(node.left,  `${id}L`) }
    if (node.right) { const rv = Number(node.right.value); if (type === "min" ? v > rv : v < rv) invalidNodeIds.add(`${id}R`); check(node.right, `${id}R`) }
  }
  check(root, "r")
  if (invalidNodeIds.size)
    violatedRules.push(type === "min" ? "Min-Heap: parent ≤ children" : "Max-Heap: parent ≥ children")
  return { valid: !violatedRules.length, violatedRules, invalidNodeIds, invalidEdgeIds: new Set() }
}

const VALIDATORS: Partial<Record<TreeType, (root: TreeNodeData) => ValidationResult>> = {
...
const RULES: Partial<Record<TreeType, string[]>> = {
  ...
  bst:           ["All left descendants < node", "All right descendants > node", "Both subtrees are also BSTs"],
  avl:           ["BST rules apply", "Balance factor (left height - right height) ≤ 1 for every node"],
  balanced:      ["Height difference between left and right subtrees ≤ 1 at every node"],
  full:          ["Every node has exactly 0 or 2 children"],
  complete:      ["All levels fully filled except possibly the last", "Last level filled left-to-right"],
...
function normalize(node: TreeNodeData, id = "r"): INode {
  return {
    ...
    children: node.children ? node.children.map((c, i) => normalize(c, `${id}C${i}`)) : undefined,
  }
}
...
function countLeaves(node: INode | undefined): number {
  ...
  if (node.children?.length) return node.children.reduce((s, c) => s + countLeaves(c), 0)
  return countLeaves(node.left) + countLeaves(node.right)
}

function layoutBinary(node: INode, depth: number, xMin: number, xMax: number, levelH: number): LNode {
  const x = (xMin + xMax) / 2
  ...
  return { ...node, x, y, left, right } as LNode
}

function layoutNary(node: INode, depth: number, xMin: number, xMax: number, levelH: number): LNode {
  const x = (xMin + xMax) / 2
  const y = depth * levelH + levelH
  const kids = node.children ?? []
  if (!kids.length) return { ...node, x, y, children: [] } as LNode
  ...
  return { ...node, x, y, children: layoutChildren } as LNode
}

function getBounds(node: LNode, r: number): { x: number; y: number; w: number; h: number } {
...
const SPEEDS = [{ label: "1×", ms: 1000 }, { label: "2×", ms: 500 }, { label: "3×", ms: 200 }]
...
function useTraversal(steps: TraversalStep[]) {
  ...
  React.useEffect(() => { setIdx(-1); setPlaying(false) }, [steps])

  React.useEffect(() => {
    if (!playing) return
    const t = setInterval(() => {
      setIdx(prev => { if (prev >= steps.length - 1) { setPlaying(false); return prev }; return prev + 1 })
    }, SPEEDS[speedIdx].ms)
  ...
  return { idx, setIdx, playing, setPlaying, speedIdx, setSpeedIdx, activeId, visitedIds, description, total: steps.length }
}

// ─── Rendering Engine ─────────────────────────────────────────────────────────

const PALETTE = {
  normal:  { fill: "var(--ds-bg, hsl(var(--background)))",   stroke: "var(--ds-border, hsl(var(--border)))",           text: "var(--ds-fg, hsl(var(--foreground)))" },
  active:  { fill: "var(--ds-primary-bg, hsl(var(--primary)/0.12))", stroke: "var(--ds-primary, hsl(var(--primary)))", text: "var(--ds-primary, hsl(var(--primary)))" },
  visited: { fill: "var(--ds-muted, hsl(var(--muted)))",     stroke: "var(--ds-muted-fg, hsl(var(--muted-foreground)/0.4))", text: "var(--ds-muted-fg, hsl(var(--muted-foreground)))" },
  invalid: { fill: "var(--ds-err-bg, hsl(var(--destructive)/0.08))", stroke: "var(--ds-err, hsl(var(--destructive)))", text: "var(--ds-err, hsl(var(--destructive)))" },
}
const EDGE_COLOR = {
...
function EdgeSvg({ from, to, r, state, avlBf }: {
  from: LNode; to: LNode; r: number
  state: "normal" | "active" | "invalid"
  avlBf?: number
}) {
  const dx = to.x - from.x, dy = to.y - from.y
  const d  = Math.sqrt(dx*dx + dy*dy) || 1
  const x1 = from.x + dx/d * r, y1 = from.y + dy/d * r
  const x2 = to.x   - dx/d * r, y2 = to.y   - dy/d * r
  const mx = (x1 + x2) / 2,     my = (y1 + y2) / 2
  return (
    <g>
      <line x1={x1} y1={y1} x2={x2} y2={y2}
        style={{ stroke: EDGE_COLOR[state], strokeWidth: state === "active" ? 2.5 : 1.5, transition: "stroke 0.3s" }} />
    </g>
  )
}

function NodeSvg({ node, r, state, showBF, balanceFactor, prefersReducedMotion }: {
  node: LNode; r: number
  state: NodeState
  showBF?: boolean
  ...
  return (
    <g role="treeitem" aria-label={`Node ${node.value}${node.annotation ? `, annotation: ${node.annotation}` : ""}`}>
      {showBF && balanceFactor !== undefined && (
        <text x={node.x + r + 4} y={node.y - r - 2}
          style={{ fontSize: 10, fill: EDGE_COLOR[Math.abs(balanceFactor) > 1 ? "invalid" : "normal"] }}
          aria-hidden="true">bf={balanceFactor}</text>
      )}
      ...
      <text x={node.x} y={node.y} textAnchor="middle" dominantBaseline="central"
        style={{ fontSize: Math.max(10, r * 0.75), fontWeight: isActive ? "700" : "400", fill: c.text, userSelect: "none",
          transition: prefersReducedMotion ? "none" : "fill 0.3s" }}>
        {node.value}
      </text>
      {node.annotation && (
        <text x={node.x} y={node.y + r + 14} textAnchor="middle"
          style={{ fontSize: 10, fill: "var(--ds-muted-fg, hsl(var(--muted-foreground)))", fontStyle: "italic" }}>
          {node.annotation}
        </text>
      )}
    </g>
  ...
  function walk(n: LNode, parentNode?: LNode) {
    if (parentNode) {
      const edgeState: "normal" | "active" | "invalid" =
        highlightInvalid && invalidIds.has(n.id) ? "invalid" :
        (activeId === n.id || activeId === parentNode.id) && visitedIds.has(n.id) ? "active" :
        "normal"
      edges.push(<EdgeSvg key={`e-${n.id}`} from={parentNode} to={n} r={r} state={edgeState} />)
    }

    const state: NodeState =
      highlightInvalid && invalidIds.has(n.id) ? "invalid" :
      n.id === activeId ? "active" :
      visitedIds.has(n.id) ? "visited" :
      "normal"

    const bf = showBF ? treeHeight(n.left) - treeHeight(n.right) : undefined

    nodes.push(
      <g key={`n-${n.id}`} style={{ cursor: onNodeClick ? "pointer" : "default" }}
        onClick={() => onNodeClick?.(n.id)}>
        <NodeSvg node={n} r={r} state={n.id === selectedId && state !== "invalid" ? "active" : state}
          showBF={showBF} balanceFactor={bf} prefersReducedMotion={prefersReducedMotion} />
      </g>
    )
...
function TraversalPanel({ traversal, onTypeChange, currentType }: {
  ...
  const btn  = "h-6 rounded px-2 text-[11px] hover:bg-muted disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
  const tbtn = (active: boolean) => cn("h-6 rounded px-2 text-[11px] border transition-colors", active ? "bg-primary text-primary-foreground border-primary" : "hover:bg-muted")

  return (
    <div className="flex flex-wrap items-center gap-1.5 border-b bg-muted/10 px-3 py-1.5">
      {types.map(t => (
        <button key={t.value} className={tbtn(currentType === t.value)}
      ...
      {currentType && (
        <>
          ...
          <button className={btn} onClick={() => { traversal.setIdx(-1); traversal.setPlaying(false) }} disabled={traversal.idx < 0}>⏮</button>
          <button className={btn} onClick={() => traversal.setIdx(i => Math.max(-1, i - 1))} disabled={traversal.idx < 0}>‹</button>
          <button className={cn(btn, "w-7 font-medium")} onClick={() => {
            if (traversal.idx >= traversal.total - 1) { traversal.setIdx(-1); traversal.setPlaying(true) }
            else traversal.setPlaying(p => !p)
          }}>{traversal.playing ? "⏸" : "▶"}</button>
          <button className={btn} onClick={() => traversal.setIdx(i => Math.min(traversal.total - 1, i + 1))} disabled={traversal.idx >= traversal.total - 1}>›</button>
          <button className={btn} onClick={() => { traversal.setIdx(traversal.total - 1); traversal.setPlaying(false) }} disabled={traversal.idx >= traversal.total - 1}>⏭</button>
          <button className={cn(btn, "font-mono ml-1")} onClick={() => traversal.setSpeedIdx(s => (s + 1) % SPEEDS.length)}>
            {SPEEDS[traversal.speedIdx].label}
          </button>
          {traversal.description && (
            <span className="text-[11px] text-muted-foreground ml-1">
              <span className="font-medium text-foreground">{traversal.idx + 1}</span>/{traversal.total} · {traversal.description}
            </span>
          )}
        </>
      )}
...
export function DSTree({
  ...
  const invalidIds = (showValidation || highlightInvalid) ? (validation?.invalidNodeIds ?? new Set()) : new Set<string>()

  // Traversal
  const [activeTraversal, setActiveTraversal] = React.useState<TraversalType | null>(defaultTraversal ?? null)
  ...
  // SVG bounds
  const bounds = React.useMemo(() => getBounds(lNode, nodeRadius), [lNode, nodeRadius])

  const { edges, nodes } = renderTree(
    lNode, nodeRadius, invalidIds,
    trav.activeId, trav.visitedIds,
  ...
  const findNode = React.useCallback((n: LNode, id: string): LNode | null => {
    if (n.id === id) return n
    if (n.left)  { const l = findNode(n.left,  id); if (l) return l }
    if (n.right) { const r = findNode(n.right, id); if (r) return r }
  ...
  return (
    <div className={cn("my-4 overflow-hidden rounded-lg border bg-background", theme === "light" && "ds-light", theme === "dark" && "ds-dark", className)}
      role="tree" aria-label={`${type} visualization`}>
      {/* Header */}
      <div className="flex items-center gap-2 border-b bg-muted/40 px-3 py-1.5">
        <span className="text-xs font-medium text-muted-foreground capitalize">
          {type === "bst" ? "BST" : type === "avl" ? "AVL Tree" : type === "nary" ? "N-ary Tree" : type.replace("-", " ")}
        </span>
        {showValidation && validation && (
          <span className={cn(
            "rounded-full px-1.5 py-0.5 text-[10px] font-semibold",
            isValid ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-400"
      ...
      {showValidation && validation && !isValid && (
        <div className="border-t bg-destructive/5 px-3 py-2">
          <p className="text-[11px] font-semibold text-destructive mb-1">Violated Rules:</p>
      ...
      {showRules && rules.length > 0 && (
        <div className="border-t bg-muted/20 px-3 py-2">
          <p className="text-[11px] font-semibold text-muted-foreground mb-1">Rules:</p>
          <ul className="space-y-0.5">
            {rules.map((r, i) => (
              <li key={i} className="text-[11px] text-muted-foreground flex gap-1.5">
                <span className="text-emerald-600">✓</span><span>{r}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      {interactive && selectedNode && (
        <div className="border-t bg-muted/10 px-3 py-2 flex flex-wrap gap-3">
          <div className="text-[11px]"><span className="text-muted-foreground">Value: </span><span className="font-semibold">{selectedNode.value}</span></div>
          <div className="text-[11px]"><span className="text-muted-foreground">Height: </span><span className="font-semibold">{treeHeight(selectedNode)}</span></div>
          {(showBalanceFactor || type === "avl") && (
            <div className="text-[11px]">
              <span className="text-muted-foreground">Balance factor: </span>
              <span className={cn("font-semibold", Math.abs(treeHeight(selectedNode.left) - treeHeight(selectedNode.right)) > 1 ? "text-destructive" : "")}>
                {treeHeight(selectedNode.left) - treeHeight(selectedNode.right)}
              </span>
            </div>
          )}
          {selectedNode.annotation && <div className="text-[11px] text-muted-foreground italic">{selectedNode.annotation}</div>}
          <button className="text-[11px] text-muted-foreground hover:text-foreground ml-auto" onClick={() => setSelectedId(null)}>✕</button>
        </div>
      )}
    </div>
  )
...
export function DSBST(props: { data: TreeNodeData } & BaseProps) {
  return <DSTree {...props} type="bst" showValidation showRules highlightInvalid />
}

export function DSAVLTree(props: { data: TreeNodeData } & BaseProps) {
  return <DSTree {...props} type="avl" showValidation showRules highlightInvalid showBalanceFactor />
}

export function DSBalancedTree(props: { data: TreeNodeData } & BaseProps) {
  return <DSTree {...props} type="balanced" showValidation showRules highlightInvalid />
}

export function DSFullTree(props: { data: TreeNodeData } & BaseProps) {
  return <DSTree {...props} type="full" showValidation showRules highlightInvalid />
}

export function DSCompleteTree(props: { data: TreeNodeData } & BaseProps) {
  return <DSTree {...props} type="complete" showValidation showRules highlightInvalid />
}

export function DSPerfectTree(props: { data: TreeNodeData } & BaseProps) {
  return <DSTree {...props} type="perfect" showValidation showRules highlightInvalid />
}
...
export function DSMinHeap({ values, ...props }: DSHeapProps) {
  ...
  const data = React.useMemo(() => arrToTreeData(heap) ?? { value: "empty" }, [heap])
  return <DSTree data={data} type="min-heap" showValidation showRules highlightInvalid {...props} />
}
...
export function DSMaxHeap({ values, ...props }: DSHeapProps) {
  ...
  const data = React.useMemo(() => arrToTreeData(heap) ?? { value: "empty" }, [heap])
  return <DSTree data={data} type="max-heap" showValidation showRules highlightInvalid {...props} />
}
 ...
 */
export function DSBSTPlayground({ initialValues = [], ...props }: DSBSTPlaygroundProps) {
  ...
  const root = React.useMemo(() =>
    values.reduce<TreeNodeData | null>((r, v) => {
      function ins(n: TreeNodeData | null, val: number): TreeNodeData {
        ...
        if (val < Number(n.value)) return { ...n, left: ins(n.left ?? null, val) }
        if (val > Number(n.value)) return { ...n, right: ins(n.right ?? null, val) }
        return n
      }
      return ins(r, v)
    }, null),
  ...
  const btnP = "h-7 rounded bg-primary px-2.5 text-xs text-primary-foreground hover:bg-primary/90 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
  const btnO = "h-7 rounded border px-2.5 text-xs hover:bg-muted disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
  const inp  = "h-7 w-24 rounded border bg-background px-2 text-xs focus:outline-none focus:ring-1 focus:ring-ring"

  return (
    <div className={cn("my-4 overflow-hidden rounded-lg border bg-background", props.className)}>
      <div className="flex flex-wrap items-center gap-1.5 border-b bg-muted/10 px-3 py-2">
        <input value={input} onChange={e => setInput(e.target.value)}
          ...
          placeholder="number…" className={inp} />
        <button onClick={insert} disabled={!input.trim() || isNaN(Number(input))} className={btnP}>Insert</button>
        <button onClick={() => setValues(v => v.slice(0, -1))} disabled={!values.length} className={btnO}>Undo Last</button>
        <button onClick={() => setValues([])} disabled={!values.length} className={btnO}>Clear</button>
        <span className="text-[11px] text-muted-foreground ml-auto">nodes: {values.length}</span>
      </div>
      {root ? (
        <DSTree data={root} type="bst" showValidation showRules highlightInvalid interactive
          className="my-0 rounded-none border-0" {...props} />
      ) : (
